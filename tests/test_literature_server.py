...
class LiteratureServerTest(unittest.TestCase):
    ...
    def test_literature_crud_rendering_sitemap_and_rss(self):
        ...
        self.assertIn(f"<title>{server.seo_title(post['title'])}</title>", text)
        ...
        self.assertIn('og:image:width" content="1200"', text)
        self.assertIn('og:image:height" content="630"', text)
        self.assertIn('og:image:alt" content="소설가 이후 공식 홈페이지 대표 이미지"', text)
        self.assertIn('twitter:image:alt" content="소설가 이후 공식 홈페이지 대표 이미지"', text)
        structured_data = json.loads(text.split('<script type="application/ld+json">', 1)[1].split('</script>', 1)[0])
        article = next(entry for entry in structured_data["@graph"] if entry["@type"] == "BlogPosting")
        self.assertEqual(article["image"]["url"], server.OG_IMAGE)
        self.assertEqual(article["image"]["width"], 1200)
        self.assertEqual(article["image"]["height"], 630)
