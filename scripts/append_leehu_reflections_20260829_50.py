...
def prose(work,motif,scene,tension,action,i):
 titles=(f'{work} 문학노트: {motif}에 머물러 본 저녁',f'{work}를 읽고 {motif}에서 발견한 질문',f'{motif}의 감각으로 다시 만나는 {work}',f'{work} 문학노트 — {motif} 뒤에 남는 것',f'오늘의 {work}: {motif}를 서두르지 않는 법',f'{motif} 앞에서 펼쳐 본 {work}',f'{work}와 {motif}, 익숙한 판단을 비켜 가기',f'한 장면의 사유: {work} 곁의 {motif}')
 decks=(f'첫 장면은 {scene}. 이 작은 {motif} 풍경은 {tension} 사이에서 무엇을 선택할지 조용히 묻는다.',f'{motif}를 떠올리면 {scene}. 작품보다 먼저 내 일상의 태도가 읽히기 시작한다.',f'오늘 《{work}》 곁에 놓인 소재는 {motif}다. {scene}, 그때 ‘{tension}’이라는 문제가 모습을 드러낸다.',f'기억에 남은 것은 {scene}. 나는 이 풍경을 통해 독서를 결론이 아니라 질문의 시간으로 바꾸어 본다.')
 starts=(f'첫 풍경은 {scene}이다. 이 장면을 작품의 줄거리로 오해할 필요는 없다.',f'{motif}라는 소재는 사소한 생활 장면에서 출발한다. 구체적으로는 {scene}이다.',f'이번 기록의 중심에는 {motif}가 있다. 내가 떠올린 것은 {scene}이다.',f'어떤 독서는 질문보다 감각으로 먼저 돌아온다. 내게는 {scene}이 그런 순간이었다.',f'눈앞의 {scene}을 평소라면 지나쳤겠지만, 풍경이 {motif}라는 이름을 얻자 오래 머문다.',f'《{work}》를 다시 생각하며 책 밖의 {motif}를 살폈다. 눈앞에는 {scene}.',f'{motif}에서 출발한 생각은 예상보다 멀리 갔다. 그 출발점은 {scene}이다.',f'책을 읽고 곧장 교훈을 만들고 싶지 않았다. 대신 {scene}을 오래 바라보았다.')
...
def vary_sentences(text,motif,index):
 prefixes=(f'{motif}를 곁에 두면, ',f'{motif}의 관점에서는, ',f'{motif}를 오래 바라보니, ',f'{motif}에서 출발하면, ',f'{motif}에 관한 이번 독서에서, ',f'{motif}의 감각을 따라가면, ',f'{motif} 앞에 잠시 멈추면, ',f'{motif}를 오늘의 질문으로 삼으면, ')
...
def make_note(w,wi,mi,seq):
 ...
 obj=motif+josa(motif,'을','를'); subj=motif+josa(motif,'이','가'); join='《'+work+'》'+josa(work,'과','와')
 seo={
 ...
 'why_read_now':f'{now}에는 ‘{tension}’이라는 문제를 한쪽 답으로 밀어붙이기 쉽다. 오늘 떠올린 풍경은 {scene}이다. 이는 《{work}》를 오늘 다시 생각하면서 {motif}를 통해 즉각적인 요약보다 맥락을 먼저 살피게 한다.',
 'personal_reflection':f'나는 {scene}을 떠올리며 판단이 얼마나 빨리 결론을 만드는지 돌아보았다. {join} {motif} 사이의 연결은 작품의 내용을 대신 설명하지 않는다. 대신 {tension} 앞에서 내가 취한 거리와 말투를 점검하게 한다.',
 ...
 note['commentary'] += f' {motif}를 살피는 동안 나는 《{work}》를 대신 설명하기보다, 이 소재가 오늘의 판단과 태도에 남기는 미세한 변화를 끝까지 관찰해야 한다고 느꼈다.'
...
def review(notes):
 ...
 for n in notes:
  ...
  if any(x in prose for x in ('AI','자동 생성','공식 카탈로그','원문 확인 필요','권리이','자유을','과정를','소나기을','경계을')): raise SystemExit('forbidden prose '+n['id'])
